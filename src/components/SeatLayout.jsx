...
export default function SeatLayout({
    ...
    const renderSeat = (seatValue, isCabin = false) => {
        ...
        const base =
            "flex items-center justify-center rounded-xl border font-medium transition-all duration-200 select-none shrink-0";

        const sizeCls = compact
            ? "h-7 min-w-[30px] px-1 text-[10px] sm:h-8 sm:min-w-[34px] sm:px-1.5 sm:text-[11px]"
            : "h-8 min-w-[32px] px-1.5 text-[11px] sm:h-9 sm:min-w-[38px] sm:px-2 sm:text-xs md:h-10 md:min-w-[42px] md:px-2 md:text-sm";
    ...
    return (
        <div className="w-full">
            <div className="mb-2 flex items-center justify-between">
                <div className="text-xs font-semibold text-slate-700 sm:text-sm md:text-base">
                    Seats ({totalSeats})
                </div>
            </div>

            <div className="rounded-3xl border border-slate-200 bg-slate-50/70 p-2 sm:p-4 md:p-5">
                <div className="overflow-x-auto">
                    <div className="min-w-[220px] sm:min-w-[280px] md:min-w-0">
                        <div className="space-y-1.5 sm:space-y-2">
                            {rows.map((row, index) => {
                                ...
                                return (
                                    <div
                                        key={index}
                                        className={`flex items-center gap-1.5 sm:gap-3 md:gap-4 ${isBack ? "justify-center" : "justify-between"
                                            }`}
                                    >
                                        {!isBack ? (
                                            <div
                                        ...
                                        {!isBack && <div className="min-w-[8px] flex-1 sm:min-w-[20px] md:min-w-[32px]" />}

                                        <div
                                            className={`flex gap-1.5 sm:gap-2 md:gap-2.5 ${isBack ? "justify-center" : "justify-end"
                                                }`}
                                        >
                ...
                {cabinSeatIds.length > 0 && (
                    <div className="mt-4 border-t border-slate-200 pt-3 sm:mt-5 sm:pt-4">
                        <div className="mb-2 text-[10px] font-semibold text-slate-700 sm:text-xs">
                            Cabin
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-6">
                            {cabinSeatIds.map((seatId) => (
                                <div key={seatId} className="flex flex-col items-center gap-1">
                                    <div className="text-[9px] font-semibold text-slate-500">CB</div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 text-[9px] sm:mt-5 sm:text-[10px] md:grid-cols-3 lg:grid-cols-5">
                    <div className="flex items-center gap-1.5">
                        <span className="h-2.5 w-2.5 rounded border border-slate-300 bg-white" />
                        Available
                    </div>
                    <div className="flex items-center gap-1.5">
                        <span className="h-2.5 w-2.5 rounded bg-[#f97316]" />
                        Selected
                    </div>
                    <div className="flex items-center gap-1.5">
                        <span className="h-2.5 w-2.5 rounded border border-red-300 bg-red-200" />
                        Booked
                    </div>
                    <div className="flex items-center gap-1.5">
                    ...
                    <div className="flex items-center gap-1.5">
                        <span className="h-2.5 w-2.5 rounded border border-amber-300 bg-amber-100" />
                        Blocked
                    </div>
                </div>
            </div>
